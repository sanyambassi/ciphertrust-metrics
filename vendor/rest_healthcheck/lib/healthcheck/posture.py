...
def collect_posture_summary(sections: list[dict]) -> dict[str, Any]:
    ...
    props = _sec_detail(by, "system_properties") or {}
    groups = _sec_detail(by, "groups") or {}
    ...
    return {
        ...
        "access": {
            ...
            "properties": {
                "result": _sec_result(by, "system_properties"),
                "modified_count": props.get("modified_count"),
                "known_defaults": props.get("known_defaults"),
            }
            if props or _sec_result(by, "system_properties")
            else None,
            "groups": {
                "result": _sec_result(by, "groups"),
                "custom_groups_count": groups.get("custom_groups_count"),
                "admin_users_count": groups.get("admin_users_count"),
                "admin_never_logged_in": groups.get("admin_never_logged_in"),
            }
            if groups or _sec_result(by, "groups")
            else None,
...
def build_posture_table(posture: dict[str, Any]) -> list[dict[str, str]]:
    ...
    props = access.get("properties") or {}
    ...
    modified_props = int(props.get("modified_count") or 0)
    custom_groups = int(groups.get("custom_groups_count") or 0)
    admin_never = int(groups.get("admin_never_logged_in") or 0)
    add(
        "Access",
        _worst_result(
            access.get("password_policies"),
            (ldap.get("result") if ldap else None),
            (props.get("result") if props else None),
            (groups.get("result") if groups else None),
        )
        or "PASS",
        _summary_lines(
            ...
            (
                _md_bold(f"{modified_props} modified system propert(ies)")
                if modified_props
                else (
                    f"{modified_props} modified system propert(ies)"
                    if props
                    else None
                )
            ),
            (
                f"{custom_groups} custom group(s), "
                f"{groups.get('admin_users_count') or 0} admin member(s)"
                if groups
                else None
            ),
            (
                _md_bold(f"{admin_never} admin never logged in")
                if admin_never
                else None
            ),
        ),
